...
function renderDetails(snapshot) {
  ...
  body.appendChild(renderIntelligence(snapshot));
...
function renderHeader(snapshot) {
  ...
  stopBtn.addEventListener('click', async () => {
    if (!confirm('Terminate the server process for this competition?')) return;
...
function renderStats(snapshot) {
  ...
  if (snapshot.oj_endpoint) {
    grid.appendChild(statCard('Hydro Base URL', snapshot.oj_endpoint));
  }
...
function renderConfigSummary(snapshot) {
  ...
  list.appendChild(summaryItem('Delivery Time Multiplier', rules.delivery_time_multiplier ?? '1'));
  list.appendChild(
    summaryItem(
      'Error Propagation',
...
function renderIntelligence(snapshot) {
  const container = element('div');
  container.innerHTML = '<h4>Intelligence Report</h4>';

  if (!snapshot.competition_id || !snapshot.server_base_url) {
    container.appendChild(
      element('p', { class: 'status-line' }, 'Intelligence report will be available after competition creation.'),
    );
    return container;
  }

  const compId = snapshot.competition_id;
  const base = snapshot.server_base_url.replace(/\/$/, '');
  const htmlUrl = `${base}/api/metrics/intelligence/${compId}?format=html`;
  const jsonUrl = `${base}/api/metrics/intelligence/${compId}?format=json`;
  const generateUrl = `${base}/api/reports/intelligence/${compId}`;

  const actions = element('div', { class: 'details__actions' });
  const openBtn = element('a', { class: 'button button--primary' }, 'Open HTML Table');
  openBtn.href = htmlUrl;
  openBtn.target = '_blank';
  openBtn.rel = 'noopener noreferrer';

  const jsonBtn = element('a', { class: 'button' }, 'Open JSON');
  jsonBtn.href = jsonUrl;
  jsonBtn.target = '_blank';
  jsonBtn.rel = 'noopener noreferrer';

  const genBtn = element('button', { class: 'button button--ghost' }, 'Generate Artifacts');
  genBtn.addEventListener('click', async () => {
    try {
      const result = await fetchJson(generateUrl);
      if (result.status === 'success') {
        alert(`Saved report to: ${result.data.output_dir}`);
      } else {
  ...
  actions.append(openBtn, jsonBtn, genBtn);
...
function buildCompetitionForm() {
  ...
  createField(scoring, {
    label: 'Delivery Time Multiplier',
    path: 'rules.delivery_time_multiplier',
    type: 'number',
    min: 0,
    step: 0.01,
  });
...
function serializeForm(form) {
    ...
    .map((row) => {
      ...
      row.querySelectorAll('input').forEach((input) => {
        ...
        if (input.type === 'number') {
          // Preserve token-limit precision by sending string for server-side parsing.
          data[input.name] = input.name === 'limit_tokens' ? raw : Number(raw);
        } else {
          data[input.name] = raw;
        }
      });
  ...
  return {
    ...
    hydro_api_token: payload.hydro_api_token || undefined,
    competitors,
  };
}
...
function coerceInputValue(input, defaultValue) {
  ...
  if (input.type === 'number') {
    if (input.dataset.configPath === 'max_tokens_per_participant') {
      return raw;
    }
