...
* Provision dedicated API server processes for each run
* Point those runs at an external Hydro deployment
...
class CompetitionProcessManager:
    ...
    def __init__(
        ...
        self.default_hydro_base_url = os.environ.get(
            "USACOA_UI_HYDRO_BASE_URL", "http://127.0.0.1:8888"
        )
        self.default_hydro_api_token = os.environ.get(
            "USACOA_UI_HYDRO_API_TOKEN", ""
        )
    ...
    def create_instance(self, payload: Dict[str, Any]) -> CompetitionInstance:
        ...
        with self._lock:
            ...
            server_config = self._build_server_config(
                payload, server_host, server_port
            )
    ...
    def _build_server_config(
        ...
        server_config.setdefault("hydro", {})
        server_config["hydro"]["base_url"] = payload.get(
            "oj_endpoint",
            self.default_hydro_base_url,
        )
        if payload.get("hydro_api_token") is not None:
            server_config["hydro"]["api_token"] = payload.get("hydro_api_token")
        elif "api_token" not in server_config["hydro"]:
            server_config["hydro"]["api_token"] = self.default_hydro_api_token
    ...
    def _build_competition_config(
        ...
        competition_config["max_tokens_per_participant"] = _parse_bounded_int(
            max_tokens,
            field_name="max_tokens_per_participant",
            default=int(max_tokens_default),
            min_value=0,
            max_value=INT64_MAX,
        )
        ...
        try:
            ...
            competition_id = organizer.create_competition(
                ...
                max_tokens_per_participant=_parse_bounded_int(
                    ...
                    field_name="max_tokens_per_participant",
                    default=int(
                        self.competition_defaults.get("max_tokens_per_participant", 1000000)
                    ),
                    min_value=0,
                    max_value=INT64_MAX,
                ),
        ...
        for spec in instance.competitor_specs:
            ...
            competitor = Competitor(
                ...
                limit_tokens=_parse_bounded_int(
                    spec.get("limit_tokens", 10000000),
                    field_name=f"limit_tokens({spec.get('name', 'anonymous')})",
                    default=10000000,
                    min_value=0,
                    max_value=INT64_MAX,
                ),
