...
class CompetitionOrganizer:
    ...
    async def _run_competitor(self, competitor: Competitor) -> Dict:
        ...
        results = {
            ...
            "consumed_credit": final_state.get(
                "consumed_credit",
                final_state.get("consumed_tokens", 0)
                + final_state.get("submission_penalty", 0),
            ),
            "elapsed_time_seconds": final_state.get("elapsed_time_seconds", 0),
            "delivery_time_seconds": final_state.get("delivery_time_seconds", 0),
            "delivery_time_multiplier": final_state.get("delivery_time_multiplier", 1.0),
            "delivery_time_credit": final_state.get("delivery_time_credit", 0.0),
        ...
        try:
            ...
            for ranking_item in rankings_data:
                if isinstance(ranking_item, dict):
                    participant_name = ranking_item.get("name")
                    status_info = competitor_status_map.get(participant_name, {
                        "is_terminated": False,
                        "termination_reason": None
                    })
                    enhanced_item = dict(ranking_item)
                    enhanced_item["is_terminated"] = status_info["is_terminated"]
                    enhanced_item["termination_reason"] = status_info["termination_reason"]
                    enhanced_rankings.append(enhanced_item)
                elif isinstance(ranking_item, list) and len(ranking_item) >= 4:
    ...
    def _process_action(self, action: Dict, competitor: Competitor) -> Dict:
        ...
        try:
            if action_type == "view_problems":
                ...
                if not problem_id or not code or not test_cases:
                    return {
                        "status": "error",
                        "data": {
                            "action": "test_code",
                            "action_result": {"error": "Missing problem_id, code or test_cases"},
                        },
                        "should_terminate": False,
                        "termination_reason": None
                    }
