...
def _resolve_competitor_value(entry: Dict, key: str, env_key_name: str) -> str:
    explicit = str(entry.get(key, "") or "").strip()
    env_name = str(entry.get(env_key_name, "") or "").strip()
    if explicit:
        return explicit
    if env_name:
...
def write_competition_id_file(output_path: Path, competition_id: str) -> None:
    """Persist competition id so reporting/timeline tooling can attach later."""

    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(f"{competition_id}\n", encoding="utf-8")
...
def create_competitors(competitors_config: Dict, competition_config: Dict, log_dir: Optional[str] = None) -> List[Competitor]:
    ...
    for competitor in competitors_config["competitors"]:
        if competitor["type"] == "generic":
            ...
            api_base_url = _resolve_competitor_value(
                competitor, "api_base_url", "api_base_url_env"
            )
            api_key = _resolve_competitor_value(
                competitor, "api_key", "api_key_env"
            )
...
async def main():
    ...
    try:
        ...
        parser.add_argument(
            '--competition-id-output',
            default='',
            help='Optional file path used to write the created competition ID immediately after setup.',
        )
        ...
        if args.log_level or args.log_dir:
            log_config = competition_config.setdefault("log", {})
            if not isinstance(log_config, dict):
                log_config = {}
                competition_config["log"] = log_config
            if args.log_level:
                log_config["level"] = args.log_level
            if args.log_dir:
                log_config["dir"] = args.log_dir
        ...
        if args.competition_id_output:
            competition_id_output = Path(args.competition_id_output).expanduser()
            write_competition_id_file(competition_id_output, competition_id)
            logger.info(f"Wrote competition ID to {competition_id_output}")
