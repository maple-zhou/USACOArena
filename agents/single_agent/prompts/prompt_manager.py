...
- Choose your programming language wisely based on the problem requirements\n\n"""
    ...
    def create_state_prompt(self, state: Dict) -> str:
        ...
        elapsed_time_seconds = competitor.get("elapsed_time_seconds")
        if elapsed_time_seconds is not None:
            prompt += (
                f"- Elapsed Time (seconds): {elapsed_time_seconds}\n"
                f"- Delivery Time Multiplier: {competitor.get('delivery_time_multiplier', 1.0)}\n\n"
            )
        ...
        def _format_ranking_line(index: int, item) -> str:
            if isinstance(item, dict):
                rank = item.get("rank", index + 1)
                name = item.get("name") or item.get("participant_name") or item.get("participant_id") or "unknown"
                score = item.get("score", item.get("problem_pass_score", 0))
                consumed = item.get("consumed_credit", item.get("tokens_used", 0))
                suffix = ""
                if "is_terminated" in item:
                    suffix = f" [{'TERMINATED' if item.get('is_terminated') else 'ACTIVE'}]"
                return f"{rank}. {name}: Score {score} points, Consumed Credit + Penalty: {consumed}{suffix}"

            if isinstance(item, (list, tuple)) and len(item) >= 3:
                name = item[0]
                score = item[1]
                consumed = item[2]
                suffix = f" [{'TERMINATED' if item[4] else 'ACTIVE'}]" if len(item) > 4 else ""
                return f"{index+1}. {name}: Score {score} points, Consumed Credit + Penalty: {consumed}{suffix}"
        ...
        if isinstance(state["rankings"], dict) and "rankings" in state["rankings"]:
            ...
            rankings = "\n".join(
                _format_ranking_line(i, r)
                for i, r in enumerate(rankings_list)
            )
        elif isinstance(state["rankings"], list):
            # Handle the case where rankings is a list of lists/objects
            rankings = "\n".join(
                _format_ranking_line(i, r)
                for i, r in enumerate(state["rankings"])
            )
        else:
            # Fallback for unexpected ranking structures
            rankings = str(state["rankings"])
