    ...
    async def generate_response(self, state: Dict, prompt: str) -> str:
        ...
        for _ in range(self.max_retries):
            try:
                ...
                formatted_body["_usacoarena_request_format"] = {
                    "url": self.request_format.get("url", "/v1/chat/completions"),
                    "method": self.request_format.get("method", "POST"),
                    "headers": dict(self.request_format.get("headers", {})),
                    "body_template": body,
                }
                formatted_body["_usacoarena_response_format"] = dict(self.response_format)
                
                # logger.error(f"request_timeout{self.request_timeout})")
